...
class SmartPrinterClient:
    ...
    async def print_expenses_range_report(self, ip: str, port: int, data: Dict, paper_width: int) -> bool:
        ...
        general -- sin depender de un cierre de caja puntual (variante de
        print_expenses_report para el boton "Imprimir reporte" del Reporte
        de Egresos).
        """
        printer = None
        try:
            ...
            self.safe_print_text(printer, "=" * 48, align='center')
            self.safe_print_text(printer, "REPORTE DE EGRESOS",
                            bold=True, double_height=True, align='center')
            self.safe_print_text(printer, "=" * 48, align='center')
            ...
            self.safe_print_text(
                printer,
                f"Del {clean_text(date_range.get('start', ''))} al {clean_text(date_range.get('end', ''))}",
                bold=True, align='left'
            )

            user = data.get('user', {})
            if user.get('name'):
                self.safe_print_text(printer, f"Impreso por: {clean_text(user.get('name', ''))}", align='left')
            ...
            for method_code, method_data in expenses_by_method:
                ...
                for item in items:
                    ...
                    amount = item.get('amount', 0)
                    time_str = clean_text(item.get('time', ''))
                    line = f"{time_str}  {desc[:26]:<26} S/ {amount:>7.2f}"
                    printer._raw(line.encode('cp850', errors='replace'))
                    printer._raw(b'\n')
                    if item.get('reference'):
                        self.safe_print_text(printer, f"   Ref: {clean_text(item['reference'])[:38]}", align='left')
            ...
            logger.info("✅ Reporte de egresos por rango impreso")
            return True

        except Exception as e:
            logger.error(f"❌ Error imprimiendo reporte de egresos por rango: {e}", exc_info=True)
            return False
        finally:
            if printer:
                try:
